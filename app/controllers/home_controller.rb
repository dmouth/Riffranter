class HomeController < ApplicationController
  def index
    if current_user
      ...
      @hot_rants = Rant.find_by_sql("SELECT rants.*, avg(votes.stars) as rating FROM rants INNER JOIN votes ON votes.rant_id = rants.id group by rants.id order by rating desc limit 5 group by rants.*, rating")
    end
  end
end
